...
> 3.  复制到usr/local目录下，使用命令`sudo mv redis-6.0.9 /usr/local`
> 4. 切换到对应的目录进行编译 `cd /usr/local/redis-6.0.9`，`sudo make test`，`sudo make install`
> 5. 启动 redis-server
> 6. 查看redis服务是否启动 ps aux | grep redis
> 7. redis-cli 连接 redis 服务  redis-cli -h 127.0.0.1 -p 6379
> 8. 停止redis 服务  shutdown redis-cli
